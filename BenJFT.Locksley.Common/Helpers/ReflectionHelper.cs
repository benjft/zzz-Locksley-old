...
public static class ReflectionHelper {
    public static IEnumerable<Type> GetSubTypesOf<TBase>(this Assembly assembly) =>
        assembly.GetTypes().Where(type => typeof(TBase).IsAssignableFrom(type) && type != typeof(TBase));

    public static IEnumerable<Type> GetSubClassesOf<TBase>(this Assembly assembly) =>
        assembly.GetSubTypesOf<TBase>().Where(type => type is {IsClass: true, IsAbstract: false});
